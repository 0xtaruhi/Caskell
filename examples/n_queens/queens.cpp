...
const auto safe = curry([](int x, const List<int> &qs, int y) {
  auto safe_rec = make_y_combinator(
      [](auto self, int x, const List<int> &qs, int y) -> bool {
        return match(qs)
            .with(value(List<int>()), [](const List<int> &) { return true; })
            .with(wildcard(), [&self, x, y](const List<int> &qs) {
              return x != qs.head() &&
                     std::abs(x - qs.head()) !=
                         std::abs(y - static_cast<int>(qs.length())) &&
                     self(x, qs.tail(), y);
            });
      });
...
const auto queens = make_y_combinator([](auto self, int n) -> List<List<int>> {
  return match(n)
      .with(value(0), [](int) { return List<List<int>>({List<int>()}); })
      .with(wildcard(), [&self](int n) {
